            ...
            <div className="mb-8">
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-gray-600 mt-2">Daily summary and activities</p>
                    </div>
                    <div className="flex gap-3">
                        <button onClick={() => report && exportReport(report, `daily-report-${date}`)} disabled={!report} className="btn-secondary flex items-center gap-2 disabled:opacity-50">
                            <Download size={18} />
                            Export
                        </button>
                ...
                <>
                    ...
                    <div className="grid grid-cols-4 gap-4 mb-8">
                        <div className="bg-gradient-to-br from-red-50 to-red-100 border border-red-200 rounded-lg p-6">
                            ...
                            <p className="text-3xl font-bold text-red-900 mt-2">{formatCurrency(report.totals?.expensesTotal || 0)}</p>
                            <p className="text-xs text-red-700 mt-2">{report.expenses?.length || 0} expenses</p>
                        </div>

                        <div className="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-lg p-6">
                            <p className="text-green-600 text-sm font-medium">Sales Revenue</p>
                            <p className="text-3xl font-bold text-green-900 mt-2">{formatCurrency(report.totals?.salesTotal || 0)}</p>
                            <p className="text-xs text-green-700 mt-2">{report.sales?.length || 0} sales</p>
                        </div>

                        <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-lg p-6">
                            <p className="text-blue-600 text-sm font-medium">Production Count</p>
                            <p className="text-3xl font-bold text-blue-900 mt-2">{report.totals?.productionCount || 0}</p>
                            <p className="text-xs text-blue-700 mt-2">{report.production?.length || 0} entries</p>
                        </div>

                        <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-lg p-6">
                            <p className="text-purple-600 text-sm font-medium">Daily Profit</p>
                            <p className={`text-3xl font-bold mt-2 ${(report.totals?.salesTotal || 0) >= (report.totals?.expensesTotal || 0) ? 'text-purple-900' : 'text-red-900'}`}>
                                {formatCurrency((report.totals?.salesTotal || 0) - (report.totals?.expensesTotal || 0))}
                            </p>
                            <p className="text-xs text-purple-700 mt-2">For {formatDate(report.date)}</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-8 mb-8">
                                        ...
                                        >
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <p className="text-xs text-gray-600">{s.customerName} • {s.quantityLitres}L</p>
                                                </div>
                        ...
                        {/* Harvests */}
                        <div className="card">
                            <h2 className="text-xl font-bold text-gray-900 mb-4">Palm Harvests ({report.harvests?.length || 0})</h2>
                                        ...
                                        >
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <p className="text-xs text-gray-600">{h.numberOfBunches} bunches • {h.totalWeightKg}kg</p>
                                                </div>
                                                <p className="text-xs font-semibold text-palm-600">{h.harvestTeam || 'Field'}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                                <p className="text-gray-500 text-sm">No harvests recorded</p>
                            )}
                        </div>

                        {/* Operations */}
                        <div className="card">
                            <h2 className="text-xl font-bold text-gray-900 mb-4">Daily Operations ({report.operations?.length || 0})</h2>
